...
class DeleteUnusedPuppyImages extends Command
{
     ...
     * @var string
     */
    protected $description = 'Clean up uploaded images that are no longer referenced by puppies.';
    ...
    public function handle()
    {
        ...
        $usedImages = Puppy::pluck('image_url')
            // adjust path to match stored images
        ...
        // Filter only .webp unused images
        $unusedWebpImages = array_filter($unusedImages, fn($image) => str_ends_with($image, '.webp'));

        $totalCount = count($unusedWebpImages);
        ...
        $firstThree = array_slice($unusedWebpImages, 0, 3);
        ...
        if ($this->confirm('Do you want to delete these unused images?')) {
            foreach ($unusedWebpImages as $image) {
                Storage::disk('public')->delete($image);
                $this->info('Deleted ' . $image);
            }
            $this->info('Unused .webp images deleted successfully.');
        } else {
